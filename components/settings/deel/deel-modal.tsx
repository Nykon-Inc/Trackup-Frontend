...
export function DeelIntegrationDialog({ open, onOpenChange, orgId }: Props) {
    ...
    const config = activeOrg?.organization?.paymentIntegrations?.find(
        p => p.provider === PaymentIntegrationProvider.DEEL
    )
    ...
    const handleReveal = () => {
        revealKey({ organizationId: orgId, provider: PaymentIntegrationProvider.DEEL }, {
            onSuccess: (data) => {
    ...
    const handleSave = () => {
        ...
        updateIntegration({
            ...
            provider: PaymentIntegrationProvider.DEEL,
            isEnabled: isEnabled,
            apiKey: token,
        }, {
            onSuccess: () => {
                setIsSuccess(true)
                toast.success("Deel integration updated successfully")
                setTimeout(() => {
                    onOpenChange(false)
                    setIsSuccess(false)
                }, 2000)
        ...
        if (!checked) {
            updateIntegration({
                ...
                onSuccess: () => {
                    setToken("")
                    setShowKey(false)
                    toast.success("Deel integration disabled")
                },
                onError: (error: any) => {
                    setIsEnabled(true) // Revert on failure
                    toast.error(error?.message || "Failed to disable Deel integration")
                }
            })
        }
    }
                    ...
                    {isSuccess ? (
                        <div className="flex flex-col items-center justify-center py-4 space-y-4 animate-in fade-in zoom-in duration-300">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-sm text-neutral-500 font-sans mt-1">Your Deel API token has been securely stored.</p>
                            </div>
                        </div>
                    ) : !isEnabled ? (
                        <div className="space-y-6">
                            <p className="text-sm text-neutral-500 leading-relaxed font-sans">
                                Connect Deel to automate global payroll and manage EOR or contractor compliance. By enabling this, you'll be using your own (BYO) Deel account.
                            </p>
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                <div className="flex items-start gap-4 p-4 bg-neutral-50 border border-neutral-100">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[12px] text-neutral-500 leading-relaxed">Your data and payments remain in your Deel account. We only interact through your authorized API token.</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-8">
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <a
                                        href="https://developer.deel.com/docs"
                                        target="_blank"
                                        rel="noreferrer"
                                ...
                                <div className="space-y-4">
                                    {[
                                        { step: 1, text: "Log in to your Deel account", link: "https://app.deel.com/login" },
                                        { step: 2, text: "Navigate to Apps & Integrations > API tokens" },
                            ...
                            <div className="space-y-4 pt-4 border-t border-neutral-100">
                                <div className="space-y-2">
                                    <Label className="text-[10px] uppercase tracking-widest text-neutral-400 font-bold">Deel API Token</Label>
